...
const displayOptions = () => {
  optionsContainer.innerHTML += `<h3>Hangman</h3>`;
...
const canvasCreator = () => {
  ...
  return { initialDrawing,rope, head, body, leftArm, rightArm, leftLeg, rightLeg };
};
const drawMan = (count) => {
  let { rope,head, body, rightLeg, leftLeg, rightArm , leftArm } = canvasCreator();
  switch (count) {
    case 1:
      rope();
      break;
...
else if (winCount == chosenWord.length)
 {
  score = 1; 
  collectUserData();
}
function collectUserData() {
    ...
    var result = (score === 1) ? 'lost' : 'won';
    var user = { name, result };
    userArray.push(user);
    localStorage.setItem('userRecords', JSON.stringify(userArray));
    console.log("Game records array:", userArray);
